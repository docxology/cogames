...
def daf_launch_sweep(
    ...
    from daf.src.eval.comparison import _compute_performance_score
...
def daf_variant_sweep(
    ...
    """Sweep over mission variants to compare policy performance.

    Evaluates a policy on the same base mission with different variant
    configurations applied.

    Args:
        policy_spec: Policy to evaluate
        base_mission: Base mission name (e.g. 'cogsguard_machina_1')
        variant_names: List of variant names to sweep (e.g. ['dark_side', 'energized'])
        episodes: Episodes per variant
        console: Optional Rich console for output

    Returns:
        SweepResult with per-variant results
    """
    if console is None:
        console = Console()

    if not variant_names:
        raise ValueError("At least one variant name required")

    console.print(f"\n[bold cyan]Variant Sweep: {base_mission}[/bold cyan]")
    console.print(f"Variants: {', '.join(variant_names)}\n")

    sweep_result = SweepResult(
        sweep_name=f"variant_sweep_{base_mission}",
    ...
    for trial_id, variant_name in enumerate(variant_names, 1):
        console.print(f"[cyan]Trial {trial_id}/{len(variant_names)}: variant={variant_name}[/cyan]")

        try:
            name, env_cfg, _ = get_mission(base_mission, variants_arg=[variant_name])
            ...
            if summaries:
                for mission_summary in summaries:
                    has_nonzero = False
                    if hasattr(mission_summary, "per_episode_per_policy_avg_rewards"):
                        for ep_idx, rewards in mission_summary.per_episode_per_policy_avg_rewards.items():
                    ...
                    if not has_nonzero and hasattr(mission_summary, "policy_summaries"):
                        if mission_summary.policy_summaries:
                            ps = mission_summary.policy_summaries[0]
                            if hasattr(ps, "avg_agent_metrics") and ps.avg_agent_metrics:
                                perf = _compute_performance_score(ps.avg_agent_metrics)
                                all_rewards = [perf] if not all_rewards else [perf] * len(all_rewards)

                    mission_results[f"{base_mission}+{variant_name}"] = float(np.mean(all_rewards)) if all_rewards else 0.0
            ...
            trial = SweepTrialResult(
                trial_id=trial_id,
                hyperparameters={"variant": variant_name},
                primary_metric=primary_metric,
                all_metrics={"avg_reward": primary_metric},
                mission_results=mission_results,
                success=primary_metric > 0,
            )

            sweep_result.add_trial(trial)
            console.print(f"  → avg_reward: {primary_metric:.4f}\n")
